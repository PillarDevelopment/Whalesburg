...
contract ElephantCrowdsale is TokenERC20 {
    ...
    uint public startICO = 1520171904; //now
    ...
    address boynty = 0xADc50Ae48B4D97a140eC0f037e85e7a0B13453C4;
    ...
    function discountDate(address _investor, uint256 amount) internal {
        ...
        _amount = _amount.add(bonusTimeTokens);
        //_amount = _amount.add(bonusTokens);
        //require(amount > avaliableSupply); // проверка что запрашиваемое количество токенов меньше чем есть на балансе
    ...
    */
    ...
    function distributionTokens() public onlyOwner {
        require(!distribute);
        _transfer(this, escrow, 30000000*DEC); // frozen all(pre-ico, team(20,5), promo(10))
        -transfer(this, boynty, 3500000*DEC); //ransfer for boynty
        avaliableSupply -= 33500000*DEC;
        distribute = true;
    }
}